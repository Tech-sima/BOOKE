...
function adaptUIForTelegram() {
    ...
    console.log('Adapting UI for Telegram Mini App');
    ...
    if (infoPanel) {
        ...
        console.log('Updated info panel position');
    }
    ...
    console.log(`Applied Telegram offset: ${telegramTopOffset}px for screen width: ${screenWidth}px`);
}
...
function updateBuildingPanelForTelegram(telegramOffset) {
    ...
    console.log(`Updated building panel bottom to ${newBottom}px for screen width: ${screenWidth}px`);
}
...
function updateDynamicBuildingPanelsForTelegram(telegramOffset) {
    ...
    buildingPanelContainers.forEach(container => {
        ...
        console.log(`Updated building panel container max-height to ${maxHeight} for screen width: ${screenWidth}px`);
    });
}
...
function enableDeepScrollForTelegram() {
    ...
    console.log('Enabling deep scroll for Telegram Mini App');
...
function fixBuildingIndicatorsForTelegram(telegramOffset) {
    ...
    console.log('Fixing building indicators positioning for Telegram Mini App');
    ...
    indicators.forEach(indicator => {
        ...
        if (currentTop < 100) { // Если индикатор в верхней части
            ...
            console.log(`Adjusted indicator position: ${currentTop + telegramOffset}px`);
        }
    });
}
...
function addTestMoney() {
    ...
    if (window.getPlayerMoney && window.setPlayerMoney) {
        ...
        console.log(`💰 Добавлено 200к! Новый баланс: ${(newMoney/1000).toFixed(1)}k`);
...
function animate() {
    ...
    // позиционируем кружки над заводом с учетом левой панели
    ...
    if(factoryObjRef && factoryProgressDiv && factoryBankDiv){
        ...
        // Движок пересечения с левой панелью
...
function renderCity() {
    const container = document.querySelector('.city-content-container');
    if (!container) {
    ...
    // Список зданий для панели города
    const buildings = [
        {
            name: 'Завод',
            image: 'assets/svg/city-panel/factory.svg',
            cost: 35000,
            description: 'Производственное здание для изготовления товаров',
            built: localStorage.getItem('factoryBuilt') === '1'
        },
        {
            name: 'Библиотека',
            image: 'assets/svg/city-panel/libery.svg',
            cost: 135000,
            description: 'Образовательное учреждение для повышения навыков',
            built: localStorage.getItem('libraryBuilt') === '1'
        },
        {
            name: 'Почта',
            image: 'assets/svg/city-panel/mail.svg',
            cost: 75000,
            description: 'Почтовое отделение для доставки корреспонденции',
            built: localStorage.getItem('mailBuilt') === '1'
        },
        {
            name: 'Типография',
            image: 'assets/svg/city-panel/typography.svg',
            cost: 200000,
            description: 'Печатное предприятие для производства печатной продукции',
            built: localStorage.getItem('typographyBuilt') === '1'
        },
        {
            name: 'Статуя',
            image: 'assets/svg/city-panel/statue.svg',
            cost: 500000,
            description: 'Памятник для повышения престижа города',
            built: localStorage.getItem('statueBuilt') === '1'
        }
    ];
    ...
    buildings.forEach(building => {
        ...
        const costFormatted = (building.cost / 1000) + 'k';
        const canAfford = getBalance() >= building.cost;
        
        item.innerHTML = `
            <img src="${building.image}" alt="${building.name}" style="width:100%;height:auto;max-height:400px;object-fit:contain;" onerror="this.style.display='none';">
        `;
...
function buildBuilding(buildingType) {
    const buildings = {
        'завод': { cost: 35000, key: 'factoryBuilt', name: 'Завод' },
        'библиотека': { cost: 135000, key: 'libraryBuilt', name: 'Библиотека' },
        'почта': { cost: 75000, key: 'mailBuilt', name: 'Почта' },
        'типография': { cost: 200000, key: 'typographyBuilt', name: 'Типография' },
        'статуя': { cost: 500000, key: 'statueBuilt', name: 'Статуя' }
    };
    
    const building = buildings[buildingType];
    if (!building) return;
    
    if (getBalance() < building.cost) {
        alert('Недостаточно денег!');
        return;
    }
    
    setBalance(getBalance() - building.cost);
    localStorage.setItem(building.key, '1');
    
    // Показываем уведомление о покупке
    showPurchaseNotification(building.name, building.cost);
...
function safeAddEventListener(elementId, event, handler) {
    ...
    if (element) {
        ...
        if (!element.hasAttribute('data-handler-added')) {
            ...
            console.log(`✅ Event handler added for ${elementId}.${event}`);
        } else {
            console.log(`⚠️ Event handler already exists for ${elementId}.${event}`);
        }
    } else {
...
function initializeShop() {
    ...
    if (shopInitialized) {
        console.log('⚠️ Shop already initialized, skipping...');
        return;
    }
    ...
    try {
        ...
        // Убираем дублирующий вызов hideProfitIndicators
...
function buyChest() {
    ...
    console.log(`Purchased ${item.name}: ${item.coins} coins for ${item.cost} ${item.isRBC ? 'RBC' : 'money'}`);
}
...
function switchSafe(direction) {
    ...
    console.log('Switched to safe:', items[newIndex].name);
}
...
function updateSafeDisplay() {
    ...
    if (safeImage) {
        ...
        // Через короткую задержку меняем изображение и показываем
        setTimeout(() => {
...
function buySafe() {
    ...
    console.log(`Purchased ${item.name}: ${item.cost} ${item.isRBC ? 'RBC' : 'money'}`);
}
...
function switchCharacter(direction) {
    ...
    console.log('Switched to character:', items[newIndex].name);
}
...
function buyCharacter() {
    ...
    console.log(`Purchased ${item.name}: ${item.cost} money, reward: ${rewards}`);
}
...
function switchChest(direction) {
    ...
    console.log('Switched to chest:', items[newIndex].name);
}
...
function shopBuyHandler(event) {
    console.log('Shop buy handler - using SVG panel');
}

function shopNavHandler(event) {
    console.log('Shop nav handler - using SVG panel');
}
...
function initializeShopCards() {
    ...
    // так как все элементы уже есть в SVG панели
    console.log('Shop cards initialization skipped - using SVG panel');
}

// Функция навигации оставлена для совместимости
function navigateShopItem(section, currentIndex, direction) {
    console.log('Navigate shop item - using SVG panel');
}
...
function updateShopCard(section, index) {
    console.log('Update shop card - using SVG panel');
}
...
function openProfilePanel() {
    console.log('openProfilePanel called');
    if (isAnyPanelOpen()) return;
    setActiveNavButton(5);
    showPanelWithAnimation('profile-panel');
...
function preloadShopImages() {
    ...
    console.log('✅ Shop images preloaded');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        if (!shopInitialized) {
            console.log('🛍️ Initializing shop system...');
            try {
                initializeShop();
